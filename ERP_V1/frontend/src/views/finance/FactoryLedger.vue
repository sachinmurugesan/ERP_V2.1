...
import { financeApi, factoriesApi } from '../../api'
...
async function loadFactories() {
  try {
    const res = await factoriesApi.list()
    factories.value = res.data.items || res.data || []
  } catch (e) { console.error(e) }
}

async function loadLedger() {
  if (!selectedFactory.value) { entries.value = []; return }
  loading.value = true
  try {
    const params = {}
    if (startDate.value) params.start_date = startDate.value
    if (endDate.value) params.end_date = endDate.value
    const res = await financeApi.factoryLedger(selectedFactory.value, params)
    entries.value = res.data.entries || []
    summary.value = res.data.summary || { total_debit: 0, total_credit: 0, net_balance: 0 }
    factoryName.value = res.data.factory_name || ''
  } catch (e) { console.error(e) }
  finally { loading.value = false }
}

watch([selectedFactory, startDate, endDate], loadLedger)
...
async function downloadStatement(format) {
  ...
  try {
    ...
    if (startDate.value) params.start_date = startDate.value
    if (endDate.value) params.end_date = endDate.value
    const res = await financeApi.downloadFactoryLedger(selectedFactory.value, format, params)
    const blob = new Blob([res.data])
    const url = window.URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = `factory_ledger_${factoryName.value || 'statement'}.${format === 'xlsx' ? 'xlsx' : 'pdf'}`
    a.click()
  ...
  <div class="p-6 space-y-5">
    ...
    <div class="flex items-end gap-4 flex-wrap">
      <div class="flex-1 min-w-[200px]">
        <label class="block text-xs font-medium text-slate-500 mb-1">Factory</label>
        <select
      ...
      <div class="flex gap-2" v-if="selectedFactory && entries.length > 0">
    ...
    <!-- No Factory Selected -->
    <div v-if="!selectedFactory" class="text-center py-16 text-slate-400">
      <i class="pi pi-building text-4xl text-slate-300 mb-3" /><br>
      <span class="text-sm">Select a factory to view their ledger</span>
    </div>
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b-2 border-slate-200">
              ...
              <th class="text-left py-3 px-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Currency</th>
              <th class="text-right py-3 px-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Forex Rate</th>
          ...
          <tbody>
            ...
            >
              ...
              <td class="py-2.5 px-3 text-slate-500 text-xs font-mono">{{ e.currency || '-' }}</td>
              <td class="py-2.5 px-3 text-right text-slate-500 text-xs">{{ e.exchange_rate ? Number(e.exchange_rate).toFixed(2) : '-' }}</td>
          ...
          <tfoot>
            <tr class="border-t-2 border-slate-300 bg-slate-50">
              <td colspan="5" class="py-3 px-3 font-bold text-slate-700">TOTALS</td>
