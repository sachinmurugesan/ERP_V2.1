...
| `/orders/{uuid}` | 2026-04-26 | feat/orders-dashboard-tab + feat/orders-files-tab | Internal order-detail shell — promoted from `/orders-shell-preview/{id}` sandbox. nginx routes UUID-shaped paths only (regex `^/orders/[0-9a-f-]{36}$`) so `/orders/new` falls through to Vue. ADMIN portal only. The 17-stage stepper + transition action bar + **2 migrated tabs** are live: **Dashboard** (6 cards: Order Summary · Factory & Costs (D-004 gated to FINANCE/SUPER_ADMIN) · Client Payments · Shipment Status · Customs/BOE · Stage Timeline mini-view) and **Files** (full CRUD: list / upload modal with doc_type select / download via useBlobDownload hook / delete with DeleteConfirmDialog — all role-gated via DOCUMENT_UPLOAD + DOCUMENT_DELETE which mirror backend documents.py:132). The remaining 12 tabs (Items · Payments · Production · Packing · Booking · Sailing · Shipping Docs · Customs · After-Sales · Final Draft · Queries · Landed Cost) render an inline "Tab content is being migrated" placeholder with TWO escape hatches: (a) "Open in legacy system →" → `/_legacy/orders/{uuid}?tab={value}` which nginx routes to vue_upstream with the prefix stripped (Vue still sees `/orders/{uuid}?tab=...` and renders OrderDetail.vue normally — no Vue source change needed), and (b) "Go to dashboard tab →" which stays inside Next.js. 7 new API proxies total: dashboard tab (payments, factory-payments, shipments, BOE — PR #4) + files tab (documents list+upload, doc download, doc delete — feat/orders-files-tab). All shapes R-19 verified live. |
